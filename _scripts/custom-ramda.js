...
if (argv.insert) {
  ...
  const ramdaMatcher = new RegExp('R\\.([_a-zA-Z]+)', 'gm')
  const methods = Array.from(new Set(flatten(argv._.map(file => fs.readFileSync(file).toString().match(ramdaMatcher)), 1)))
  if (methods.length > 0) {
    console.log(execSync(`npx --no-install @oncomouse/custom-ramda ${methods.filter(function (x) {return x !== null}).map(x => x.replace(/R./, '')).join(' ')}`).toString())
  }
}
